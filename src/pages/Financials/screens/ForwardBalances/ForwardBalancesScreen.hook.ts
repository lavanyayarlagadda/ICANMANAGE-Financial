...
export const useForwardBalancesScreen = ({ skip = false }: { skip?: boolean } = {}) => {
  ...
  const payerOptions = useMemo(
    () => filterData?.data?.payerNames?.map((p) => ({ label: p, value: p })) || [],
    [filterData],
  );
  ...
  }, [actionTriggers.export, handleExport, skip]);
  ...
  }, [actionTriggers.print, handleExport, skip]);
  ...
  const stats = useMemo(() => {
    ...
    const totalOriginalAmount = forwardBalanceNotices.reduce(
      (sum, r) => sum + (r.originalAmount ?? 0),
      0,
    );
    const totalRemainingBalance = forwardBalanceNotices.reduce(
...
export const useForwardBalanceNoticesTable = () => {
  ...
  const [noticeDetails, setNoticeDetails] = useState<
    Record<string, ForwardBalanceDetailsResponse['data']>
  >({});
  const [loadingDetails, setLoadingDetails] = useState<Set<string>>(new Set());
  const [triggerGetDetails] = useLazyGetForwardBalanceDetailsQuery();

  const toggleRow = useCallback(
    async (id: string, noticeId: string, initialOffsets?: OffsetEvent[]) => {
      setExpandedRows((prev) => {
        ...
        if (isExpanding) {
          ...
          if (!noticeDetails[id] && !loadingDetails.has(id) && !hasInitialData) {
            ...
            triggerGetDetails(noticeId)
              .unwrap()
              .then((res: ForwardBalanceDetailsResponse) => {
                if (res?.data) {
