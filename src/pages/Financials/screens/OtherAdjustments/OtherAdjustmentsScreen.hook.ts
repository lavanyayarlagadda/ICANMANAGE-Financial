...
export const useOtherAdjustmentsScreen = ({ skip = false }: { skip?: boolean } = {}) => {
  ...
  } = useGetRecoupmentFiltersQuery(undefined, { skip: isActualSkip });
  const payerOptions = useMemo(
    () =>
      filtersData?.data?.map((p) => ({
        label: p.payer,
        value: p.payer,
      })) ?? [],
    [filtersData],
  ...
  const handleExport = useCallback(
    async (exportFormat: typeof EXPORT_FORMATS.PDF | typeof EXPORT_FORMATS.XLSX) => {
  ...
  const handleDrillDown = useCallback(
    async (row: OtherAdjustmentRecord) => {
      try {
  ...
  const handleEdit = useCallback(
    (r: OtherAdjustmentRecord) => dispatch(openEditDialog(r)),
    [dispatch],
  );
  const handleDelete = useCallback(
    (id: string) => dispatch(openConfirmDelete({ id, type: 'adjustment' })),
    [dispatch],
  );
