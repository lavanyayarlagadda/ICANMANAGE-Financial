...
import { setIsDrillingDown as setGlobalDrillingDown } from '@/store/slices/uiSlice';
...
export const useAllTransactionsScreen = ({ skip = false }: { skip?: boolean } = {}) => {
  ...
  const {
    ...
    isError: filterError,
  } = useGetAllTransactionsFiltersQuery(undefined, { skip: isActualSkip });
  ...
  const transactionTypeOptions = useMemo(
    () =>
      filterData?.data?.transactionTypes?.map((t) => ({
        label: t.transactionType,
        value: String(t.transactionTypeId),
      })) ?? [],
    [filterData],
  );
  ...
  const handleDelete = useCallback(
    (id: string) => dispatch(openConfirmDelete({ id, type: 'transaction' })),
    [dispatch],
  );
