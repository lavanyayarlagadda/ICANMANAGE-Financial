...
export const usePipScreen = ({ skip = false }: { skip?: boolean } = {}) => {
  ...
  const [appliedSearchFilters, setAppliedSearchFilters] =
    useState<PipSearchFilters>(EMPTY_PIP_SEARCH_FILTERS);
  ...
  const toggleRow = useCallback(
    async (row: PipRecord, e: React.MouseEvent) => {
      e.stopPropagation();
      ...
      const hasDetails =
        (detailsByPtan[row.ptan]?.length ?? 0) > 0 || (row.npiDetails?.length ?? 0) > 0;
  ...
  const handleFilterChange = useCallback((filters: Record<string, string>) => {
    setQueryParams((prev) => {
      ...
      const isChanged =
        prev.status !== next.status || prev.payer !== next.payer || prev.category !== next.category;
