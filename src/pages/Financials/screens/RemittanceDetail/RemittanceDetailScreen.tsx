...
const RemittanceDetailScreen: React.FC = () => {
    ...
    const serviceLineColumns = useMemo<DataColumn<ServiceLine>[]>(() => [
        ...
        { id: 'units', label: 'UNITS', align: 'center', render: (r) => r.units, accessor: (r) => r.units },
        {
            id: 'charge', label: 'CHARGE', align: 'center',
            render: (r) => <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>{formatCurrency(r.charge)}</Typography>,
            accessor: (r) => r.charge,
        },
        {
            id: 'allowed', label: 'ALLOWED', align: 'center',
        ...
        {
            id: 'adjAmt', label: 'ADJ AMT', align: 'center',
        ...
        <ScreenWrapper>
            ...
            <Box sx={{ mt: 4, mb: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                {(isSlFetching || isSlLoading) && (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, bgcolor: theme.palette.action.hover, px: 1.5, py: 0.5, borderRadius: 1 }}>
