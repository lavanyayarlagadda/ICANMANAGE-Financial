...
const RemittanceDetailScreen: React.FC = () => {
  ...
  const serviceLineColumns = useMemo<DataColumn<ServiceLine>[]>(
    () => [
      ...
      },
      {
        ...
        label: 'PROC CODE',
        render: (r) => r.procCode,
        accessor: (r) => r.procCode,
      },
      {
        id: 'modifiers',
        label: 'MODIFIERS',
        render: (r) => r.modifiers || '–',
      ...
      { id: 'revCode', label: 'REV CODE', render: (r) => r.revCode, accessor: (r) => r.revCode },
      {
        id: 'dosStart',
        label: 'DOS START',
        render: (r) => formatDate(r.dosStart),
        accessor: (r) => r.dosStart,
      },
      {
        id: 'dosEnd',
        label: 'DOS END',
        render: (r) => formatDate(r.dosEnd),
        accessor: (r) => r.dosEnd,
      },
      {
        id: 'units',
  ...
  if (!detail && (!claims || claims.length === 0))
    return <Typography>No remittance detail selected.</Typography>;
      ...
      {detail && (
        <DetailCard
          sections={[
            {
              fields: [
                { label: 'Payment Date', value: formatDate(detail.paymentDate) },
                {
                  label: 'Check/EFT Number',
                  value: <MultiValueDisplay value={detail.transactionNo} displayCount={3} />,
                },
                ...
                {
                  label: 'Patient CTL No',
                  value: detail.patientCtlNo || detail.patientCtrlNo || '–',
                },
              ],
            },
            {
