...
export const useRemittanceDetailScreen = () => {
  ...
  const effectiveClaims = useMemo(() => {
    const source =
      claimsData && Array.isArray(claimsData) ? claimsData : Array.isArray(claims) ? claims : [];
    return source;
  }, [claimsData, claims]);
  ...
  const {
    data: slData,
    isFetching: isSlFetching,
    isLoading: isSlLoading,
    isError: isSlError,
  ...
  const handleClaimsPageChange = useCallback(
    (p: number) => setClaimsQueryParams((prev) => ({ ...prev, page: p })),
    [],
  );

  const handlePageChange = useCallback(
    (p: number) => setSlQueryParams((prev) => ({ ...prev, page: p })),
    [],
  );
  const handleRowsPerPageChange = useCallback(
    (s: number) => setSlQueryParams((prev) => ({ ...prev, size: s, page: 0 })),
    [],
  );
  const handleSortChange = useCallback(
    (col: string, dir: string) =>
      setSlQueryParams((prev) => ({ ...prev, sort: col, desc: dir === SORT_ORDER.DESC, page: 0 })),
    [],
  );
