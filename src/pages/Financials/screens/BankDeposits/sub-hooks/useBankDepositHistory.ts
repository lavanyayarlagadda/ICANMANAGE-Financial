...
export const useBankDepositHistory = (selectedTenant: { displayName?: string } | undefined) => {
  ...
  const [rowHistory, setRowHistory] = useState<
    Record<string, { data: RowHistoryData; isLoading: boolean }>
  >({});
  const [triggerGetHistory] = useLazyGetBaiTriggerHistoryQuery();

  const fetchRowHistory = useCallback(
    async (transactionNo: string) => {
      if (rowHistory[transactionNo]?.data) return;

      setRowHistory((prev) => ({
        ...prev,
        [transactionNo]: { ...prev[transactionNo], isLoading: true },
      }));
      try {
        ...
        setRowHistory((prev) => ({
          ...prev,
          [transactionNo]: { ...prev[transactionNo], isLoading: false },
        }));
      }
    },
