...
export const ensureForecastColumns = (columns: TrendColumn[], forecastWindow: string): TrendColumn[] => {
...
export const calculateMomFromSeries = (values: number[]): { percent: number; direction: 'UP' | 'DOWN' | 'NONE' } | null => {
...
export const normalizeTrendRow = (r: unknown, columns: TrendColumn[], rowId: string): SectionRow | null => {
...
export const normalizeFlatTrendTable = (source: GenericRecord, forecastWindow: string): TrendTableData => {
...
export const normalizeGroupedTrendTable = (source: GenericRecord, forecastWindow: string): TrendTableData => {
  const apiColumns = normalizeTrendColumns(source);
...
export const deltaColor = (delta: string | number, positive: string, negative: string, neutral: string) => {
  const deltaNum = typeof delta === 'number' ? delta : Number(String(delta).replace(/[^\d.-]/g, ''));
