...
} from '@/pages/Financials/screens/Trends/helpers/depositReconciliationHelpers';
import type {
  TrendColumn,
  GenericRecord,
} from '@/pages/Financials/screens/Trends/helpers/depositReconTypes';
...
export const useDepositReconColumnsForPreferences = (skip: boolean) => {
  ...
  const { data: adjustedCashResponse } = useGetDepositReconciliationAdjustedCashDepositQuery(
    trendsQueryParams,
    { skip },
  );
  const { data: reconciledResponse } = useGetDepositReconciliationPostedEmrReconciledQuery(
    trendsQueryParams,
    { skip },
  );
  const { data: unreconciledResponse } = useGetDepositReconciliationPostedEmrUnreconciledQuery(
    trendsQueryParams,
    { skip },
  );
  ...
  const columnsMap = useMemo(() => {
    ...
    const adjustedCashData =
      adjustedCashResponse && 'data' in adjustedCashResponse
        ? adjustedCashResponse.data
        : adjustedCashResponse;
    const reconciledData =
      reconciledResponse && 'data' in reconciledResponse
        ? reconciledResponse.data
        : reconciledResponse;
    const unreconciledData =
      unreconciledResponse && 'data' in unreconciledResponse
        ? unreconciledResponse.data
    ...
    const adjustedCashTable = normalizeFlatTrendTable(
      (adjustedCashData || {}) as GenericRecord,
      '3m',
    );
    ...
    const notPostedTable = normalizeGroupedTrendTable(
      (unreconciledData || {}) as GenericRecord,
      '3m',
    );
