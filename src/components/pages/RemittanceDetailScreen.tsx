...
const RemittanceDetailScreen: React.FC = () => {
  ...
  const claims = useAppSelector((s) => s.financials.remittanceClaims) as RemittanceDetail[];
  ...
  // Sync service line fetching state to global loader
  React.useEffect(() => {
    dispatch(setIsGlobalFetching(isSlFetching));
    return () => {
