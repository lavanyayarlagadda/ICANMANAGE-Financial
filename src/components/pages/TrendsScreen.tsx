...
const TrendsScreen: React.FC = () => {
  ...
  const { data: dashboardData, isFetching: isFetchingDashboard, refetch: refetchDashboard } = useGetForecastDashboardQuery(queryParams, { skip: activeSubTab !== 0 });
    ...
    if (actionTriggers.reload > reloadCount.current) {
      const doReload = async () => {
        try {
          ...
          if (activeSubTab === 0) {
            await Promise.all([refetchForecast(), refetchRecon(), refetchDashboard()]);
  ...
  const renderForecastTrends = () => {
    ...
    const chartData = (reconPerformance?.data || []).map(d => ({
      ...
      actual: d.actualReconciledAmount ? parseFloat(d.actualReconciledAmount) : null,
      forecast: d.forecastAmount ? parseFloat(d.forecastAmount) : null,
    }));

    const teamColumns: DataColumn<TeamPerformanceRow>[] = [
      {
        id: 'team',
        label: 'TEAM',
        minWidth: 150,
        ...
        <Box sx={{ p: 3, backgroundColor: '#fff', borderRadius: 2, mb: 3, border: `1px solid ${theme.palette.divider}` }}>
          <ResponsiveContainer width="100%" height={350}>
            <ComposedChart data={chartData}>
              ...
              />
              <Tooltip
                formatter={(v: number) => [formatCurrency(Number(v)), 'Amount']}
