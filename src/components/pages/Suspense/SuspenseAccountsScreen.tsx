        ...
        <Box sx={{ border: '1px solid #e2e8f0', borderRadius: '8px', overflow: 'hidden' }}>
            ...
            {data.map((row, idx) => (
                <Box key={idx} sx={{ ...styles.tableGridStyles(cols), borderBottom: idx < data.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                    ...
                    {type !== 'month' && (
                        <Typography sx={{ fontSize: '12px', color: '#0284c7', fontWeight: 600, textAlign: type === 'account' ? 'left' : 'center' }}>
                            {row.items}
                        </Typography>
                    )}

                    {Object.keys(row).filter(key => !['account', 'items', 'payer', 'month', 'total'].includes(key)).map(key => (
                        <Typography key={key} sx={{ fontSize: '12px', color: '#64748b', textAlign: 'right' }}>
