...
export const exportToCSV = <T>(data: T[], columns: ExportColumn<T>[], title: string) => {
  const headers = columns.map(
    (c) => c.exportLabel || (typeof c.label === 'string' ? c.label : String(c.id)),
  );
...
export const exportToPDF = <T>(data: T[], columns: ExportColumn<T>[], title: string) => {
  const headers = columns.map(
    (c) => c.exportLabel || (typeof c.label === 'string' ? c.label : String(c.id)),
  );
  const rows = data.map((row) =>
