...
export const getNavigationStructure = (
  ...
  menus.forEach((menu, menuIdx) => {
    ...
    if (!isAccessible(menu.menuName)) {
      ...
      const hasAccessibleModule = menu.modules?.some(
        (mod) =>
          isAccessible(mod.menuName) || mod.subModules?.some((sub) => isAccessible(sub.menuName)),
      );
      if (!hasAccessibleModule) return;
    }

    const config = NAV_CONFIG[menu.menuName] || {
      path: `/${menu.menuName.toLowerCase().replace(/\s+/g, '-')}`,
    };
    ...
    if (menu.menuName === 'Financials' && menu.modules) {
      financialsTabs = menu.modules
        ...
        .map((mod, modIdx) => {
          ...
          const modConfig = NAV_CONFIG[modName] || {
            path: `${config.path}/${mod.menuName.toLowerCase().replace(/\s+/g, '-')}`,
          };
          ...
          let subTabs: DynamicTab[] = mod.subModules
            ? mod.subModules
                ...
                .map((sub, subIdx): DynamicTab => {
                  ...
                  const subConfig = NAV_CONFIG[subName] || {
                    path: `${modConfig.path}/${sub.menuName.toLowerCase().replace(/\s+/g, '-')}`,
                  };
                  return {
                    id: subIdx,
                    label: subName,
          ...
          if (subTabs.length === 0 && isTrendsForecastModule) {
            const depositReconConfig = NAV_CONFIG['Deposit Reconciliation'] || {
              path: '/financials/trends-forecast/forecast/deposit-reconciliation',
            };
            const depositReconciliationPath =
              '/financials/trends-forecast/forecast/deposit-reconciliation';
            ...
            subTabs = fallbacks.filter(
              (f) => isAccessible(f.label) || isAccessible('Trends & Forecast'),
            );
          }
            ...
            const fallbacks: DynamicTab[] = [
              {
                ...
                status: 'Active',
                component: NAV_CONFIG['PIP']?.component,
              },
              {
                id: 1,
                label: 'Forward Balances',
                path: '/financials/statements/forward-balance',
                status: 'Active',
                component: NAV_CONFIG['Forward Balances']?.component,
              },
              {
                id: 2,
                ...
                status: 'Active',
                component: NAV_CONFIG['Suspense Accounts']?.component,
              },
            ];
            subTabs = fallbacks.filter((f) => isAccessible(f.label));
          }
          ...
          const hasForwardBalances =
            subTabs.some((st) => st.label === 'Forward Balances') ||
            isAccessible('Forward Balances');
          if (hasForwardBalances && mod.menuName === 'Statements') {
            if (!subTabs.some((st) => st.label === 'Forward Balances & Recoupments')) {
              subTabs.push({
                ...
                path:
                  NAV_CONFIG['Forward Balances & Recoupments']?.path ||
                  '/financials/statements/fb&recoup',
                status: 'Active',
